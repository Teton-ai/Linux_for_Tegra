...
def generateBlob(content, fv, key_ek, key_ak):
    ...
    header = struct.pack(
        ekb_header_fmt, eks_len, b"NVEKBP\0\0", version_major, version_minor
    )
...
def nist_sp_800_108_with_CMAC(key, context=b"", label=b"", len=16):
    ...
    for count in range(ceil(len / 16)):
        data = b"".join(
            [
                bytes([count + 1]),
                label.encode(encoding="utf8"),
                bytes([0]),
                context.encode(encoding="utf8"),
                int(len * 8).to_bytes(4, byteorder="big"),
            ]
        )
        c = cmac.CMAC(algorithms.AES(key), backend=default_backend())
        c.update(data)
...
def main():
    ...
    parser.add_argument(
        "-chip", nargs=1, required=False, help="specify chip: t194|t234. default: t194"
    )
    parser.add_argument(
        "-kek2_key",
        nargs=1,
        required=False,
        help="kek2 key (16 bytes) file in hex format [t194 only]",
    )
    parser.add_argument(
        "-oem_k1_key",
        nargs=1,
        required=False,
        help="oem_k1 key (32 bytes) file in hex format [t234 only]",
    )
    parser.add_argument(
        "-oem_k2_key",
        nargs=1,
        required=False,
        help="oem_k2 key (32 bytes) file in hex format [t234 only]",
    )
    parser.add_argument(
        "-in_sym_key",
        nargs=1,
        required=False,
        help="symmetric key file in hex format [t194: 16 bytes | t234: 32 bytes]",
    )
    parser.add_argument(
        "-in_sym_key2",
        nargs=1,
        required=False,
        help="16-byte symmetric key file in hex format",
    )
    parser.add_argument(
        "-in_auth_key",
        nargs=1,
    ...
    parser.add_argument(
        "-in_ftpm_sn", nargs=1, required=False, help="fTPM serial number"
    )
    parser.add_argument(
        "-in_ftpm_eps_seed",
        nargs=1,
        required=False,
        help="fTPM EPS seed file in hex format",
    )
    parser.add_argument(
        "-in_ftpm_rsa_ek_cert",
        nargs=1,
        required=False,
        help="fTPM RSA EK Cert in DER format",
    )
    parser.add_argument(
        "-in_ftpm_ec_ek_cert",
        nargs=1,
        required=False,
        help="fTPM EC EK Cert in DER format",
    )
    parser.add_argument(
        "-in_sid_cert",
        nargs=1,
        required=False,
        help="Device Silicon ID Cert in DER format",
    )
    parser.add_argument(
        "-in_ftpm_rsa_ek_csr",
        nargs=1,
        required=False,
        help="fTPM RSA EK CSR in DER format",
    )
    parser.add_argument(
        "-in_ftpm_ec_ek_csr",
        nargs=1,
        required=False,
        help="fTPM EK EK CSR in DER format",
    )
    parser.add_argument(
        "-out", nargs=1, required=True, help="where the eks image file is stored"
    )
